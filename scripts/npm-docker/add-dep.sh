...
echo -e "\033[1;36mStopping running containers...\033[0m"
docker stop $(docker ps -a -q --filter status=running)

# Enter into the Docker container in bash and run npm install --save/dev
# This exercise just ensures that we get the most up to date version of the 
...
# Now that package.json has been updated, let's clean up our image/container/volume and rebuild

echo -e "\033[1;36mRemoving existing image from local environment...\033[0m"
docker image rm rebelsnet/krebel-dependencies --force
echo -e "\033[1;36mRemoving existing containers from local environment...\033[0m"
docker rm $(docker ps -a -q -f 'name=krebel')
echo -e "\033[1;36mRemoving existing volume from local environment...\033[0m"
docker volume rm krebel_dev-db-volume --force
echo -e "\033[1;36mRebuilding image with updated package.json...\033[0m"
docker build -t rebelsnet/krebel-dependencies -f Dockerfile-dev .
echo -e "\033[1;32mDone!\033[0m"
echo -e "\033[1;33mREMINDER: 'docker push rebelsnet/krebel-dependencies' once you're satisfied with the new dependencies. This will push your image to DockerCloud for everyone else to use.\033[0m"